...
import { UserAvatar } from "@/frontend/components/auth/user/user-avatar";
...
export function ChangeAvatar({ className }: ChangeAvatarProps) {
    ...
    async function handleFileChange(e: ChangeEvent<HTMLInputElement>) {
        ...
        try {
            const resized =
                (await avatar.resize?.(file, avatar.size, avatar.extension)) ||
                file;
            ...
            updateUser(
                { image },
                {
                    onSuccess: () =>
                        toast.success(
                            localization.settings.avatarChangedSuccess,
                        ),
                },
            );
        } catch (error) {
